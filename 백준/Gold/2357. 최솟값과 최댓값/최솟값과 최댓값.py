...
def fmax(node,s,e):
  ...
  if s>r or e<l:
    return 0
  if s<=l and e>=r:
    return max_seg[node][0]
  return max(fmax(node*2+1,s,e),fmax(node*2 + 2, s,e))
...
for i in range(2**(h-1)):
  ...
  if i<n:
    ...
    max_seg[idx] = [a,idx,idx]
  else:
    min_seg[idx] = [10**10,idx,idx]
    max_seg[idx] = [0,idx,idx]
#세그먼트 생성
temp = h
while temp-1:
  s = 2**(temp-1) - 1
  ...
  for i in range(s,e+1,2):
    max_seg[i//2] = [max(max_seg[i][0],max_seg[i+1][0]),max_seg[i][1],max_seg[i+1][2]] 
